...
var render=function(){
    for(var i=0;i<_records.length;i++){
        ...
        _records[i].vm_valid={};
        _records[i].vm_custom={};
        _records[i].vm_readonly={};
        _records[i].vm_validation={};
    }
    ...
    for(var i=0;i<cols.length;i++){
        var th=cols[i];
        var thA=th.split('|')[0].replace(/_/g,' ');
        var thB=th.split('|').pop();
        if(thA!='Hidden') headerA.push(thA);
        if(thA!='Hidden') headerB.push(thB);
    }
    ...
    for(var i=0;i<_records.length;i++){
        txt+="<tr>";
        for(var j=0;j<headerB.length;j++){
            var value="";
            if(_records[i][headerB[j]]!==undefined) value=_records[i][headerB[j]];
            var value=$('<div/>').text(value).html();
            value=value.replace(/\r/g,'\n').replace(/\n\n/g,'\n').replace(/\n/g,'<br>');
            txt+="<td contenteditable>"+value+"</td>";
        }
        txt+"</tr>";
    }
    $('#grid__ID').html(txt);
    $('#grid__ID td').each(function(){
        ...
        var column_name=headerB[col];
        if(column_name=='Delete'){
            $(this).data('ID',_records[row-1].ID);
        ...
        if(column_name=='Form'){
            ...
            $(this).on('click',function(){
                var name='grid_form';
                $vm.module_list[name]=[_db_pid.toString(),'__BASE__/vmiis/Common-Code/form/grid_form.html',''];
                //var record=_records[row-1];
                $vm.load_module_by_name(name,$vm.root_layout_content_slot,
                    {records:_records,I:row-1,headerA:headerA,headerB:headerB,data_process:_data_process,widthA:_widthA,widthB:_widthB,before_submit:_before_submit}
                );
            })
        }
        if(_data_process!==''){ _data_process(_records,row-1,column_name,$(this)); }
        if(column_name=='DateTime' || column_name=='Author' || _records[row-1].vm_readonly[column_name]===true || _records[row-1].vm_custom[column_name]===true){
            $(this).removeAttr('contenteditable');
            $(this).css('background-color','#F8F8F8')
        }
    })

    $('#grid__ID td').blur(function(){
        var col = $(this).parent().children().index($(this));
        var row = $(this).parent().parent().children().index($(this).parent());
        var column_name=headerB[col];
        if(column_name=='Form' || column_name=='Delete'){
            return;
        }
        var value=$(this).html().replace(/<div>/g,'').replace(/<\/div>/g,'\n').replace(/<br>/g,'\n');
        var value=$('<div/>').html(value).text();
        if(_records[row-1].vm_custom[column_name]!==true){
            if(value!==_records[row-1][column_name]){
                _records[row-1].vm_dirty=1;
                _records[row-1][column_name]=value;
                $('#save__ID').css('background','#E00');
            }
        }
        var fun=_records[row-1].vm_validation[column_name];
        if(fun!==undefined){
            $(this).css('background','#FFF');
            _records[row-1].vm_valid[column_name]=1;
            var R=fun(value);
            if(R===false){
                $(this).css('background','#E4CDCD');
                _records[row-1].vm_valid[column_name]=0;
            }
        }
    })
}
                        ...
                        if( i<lines.length ) {
                            ...
                            if(items.length>=2 || (items.length==1 && items[0]!=='') ){
                                ...
                                if(Array.isArray($vm.module_list['import_dialog_module'])===true){
                                    ...
                                    var mid=$vm.module_list['import_dialog_module']['table_id'];
                                	var url=$vm.module_list['import_dialog_module']['url'];
            					}
...
$('#new__ID').on('click', function(){
    var new_records;
    var new_row={}
    for(var i=0;i<headerB.length;i++){
        new_row[headerB[i]]="";
    }
...
//-----------------------------------------------
$('#D__ID').on('back',function(){  _request_data();  })
