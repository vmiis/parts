...
var _set_req=function(){
	...
	//if($vm.third_party!=1)       _req={cmd:'query_records',db_pid:_db_pid,sql:sql,sql_n:sql_n,s1:'"'+$('#keyword__ID').val()+'"',I:$('#I__ID').text(),page_size:$('#page_size__ID').val()}
	//else if($vm.third_party==1)  _req={cmd:'query_records_third',db_pid:_db_pid,sql:sql,sql_n:sql_n,s1:'"'+$('#keyword__ID').val()+'"',I:$('#I__ID').text(),page_size:$('#page_size__ID').val()}
}
//-------------------------------------
var _set_req_export=function(i1,i2){
    ...
    */
	...
	//if($vm.third_party!=1)       _req={cmd:'query_records',sql:sql,i1:i1,i2:i2};
	//else if($vm.third_party==1)  _req={cmd:'query_records_third',sql:sql,i1:i1,i2:i2};
...
var _request_data=function(){
	...
	if($vm.third_party!=1){
		...
		$vm.post_message_from_child_to_parent({data:_req,origin:'*',callback:function(res){
	        if(_busy_query!=='') $vm.close_dialog({name:'busy_dialog_module'});
	        $("#I__ID").text(res.I);
	        $("#A__ID").text(res.A);
	        var mt2=new Date().getTime();
...
$('#export__ID').on('click',function(){   _export_data(); })
